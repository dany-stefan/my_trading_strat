...
# =============================================================================
# MARKET TIMING STRATEGY v1 (Sell on dips - FAILED)
# =============================================================================
def simulate_market_timing_v1(rsi_threshold=45):
    """
    Market Timing Strategy v1 - FAILED APPROACH:
    Sells 50% when RSI drops below threshold (sells on dips - WRONG!)
    """
    shares = 0.0
    cash_pool = 0.0
    cost = 0.0
    contributions = INITIAL_LUMP_SUM
    in_rainy_period = False
    rainy_cash_available = 0.0
    sell_transactions = 0
    ...
    for dt, row in prices.iterrows():
        ...
        # Bi-weekly payday: base investment + cash savings (ALWAYS)
        if dt in biweekly_set:
            if price_cad > 0:
                shares += DCA_BASE_AMOUNT / price_cad
                cost += DCA_BASE_AMOUNT
        ...
        # Market timing logic: Check for RSI threshold breach
        if pd.notna(rsi_val):
            # Entering rainy period - SELL 50% of position
            if not in_rainy_period and rsi_val < rsi_threshold:
                in_rainy_period = True
                if shares > 0:
                    shares_to_sell = shares * 0.5
                    proceeds = shares_to_sell * price_cad
                    shares -= shares_to_sell
                    rainy_cash_available = proceeds
                    sell_transactions += 1
            
            # Exiting rainy period - RSI back above threshold
            elif in_rainy_period and rsi_val >= rsi_threshold:
                in_rainy_period = False
                rainy_cash_available = 0.0
        
        # Weekly DCA during rainy period with proceeds from sale
        if in_rainy_period and dt in weekly_set:
            if rainy_cash_available > 0:
                weekly_amount = min(150.0, rainy_cash_available)
                if price_cad > 0:
                    shares += weekly_amount / price_cad
                    rainy_cash_available -= weekly_amount
        
        # Track equity
        total_equity = shares * price_cad + cash_pool + rainy_cash_available
    ...
    cagr = (end_val / contributions) ** (1 / years) - 1
    ...
    return {
        "strategy": "Market Timing v1 (Sell on dips)",
        "rsi_threshold": rsi_threshold,
        "contributions": contributions,
        "end_equity": end_val,
        "cagr": cagr,
...
# =============================================================================
# MARKET TIMING STRATEGY v2 (Buy low RSI<45, NEVER SELL)
# =============================================================================
def simulate_market_timing_v2(sell_threshold=70, buy_threshold=45):
    """
    Market Timing Strategy v2 - BUY ONLY (No Selling):
    
    SELL RULE:
    - DISABLED - Never sell, stay fully invested
    
    BUY RULE:
    - When RSI < 45 (undervalued) → Buy extra $150
    - Uses bi-weekly cash pool (same as rainy day strategy)
    
    REGULAR DCA:
    - $150 bi-weekly (1st & 15th) continues always
    - $30 bi-weekly savings to cash pool
    
    This should match your rainy day strategy exactly!
    """
    shares = 0.0
    cash_pool = 0.0
    cost = 0.0
    ...
    for dt, row in prices.iterrows():
        ...
        # Bi-weekly payday: base $150 investment + $30 cash savings
        if dt in biweekly_set:
            if price_cad > 0:
                shares += DCA_BASE_AMOUNT / price_cad
                cost += DCA_BASE_AMOUNT
        ...
        # Rainy day buying logic (RSI < 45)
        if pd.notna(rsi_val):
            # Check if we're entering or in rainy period
            if rsi_val < buy_threshold:
                in_rainy_period = True
            else:
                in_rainy_period = False
        
        # On bi-weekly payday during rainy period: deploy extra $150 from cash pool
        if dt in biweekly_set and in_rainy_period:
            if cash_pool >= 150 and price_cad > 0:
                shares += 150 / price_cad
                cash_pool -= 150
                cost += 150
                contributions += 150  # Count this as additional contribution
                total_rainy_buys += 150
                rainy_buy_count += 1
                buy_transactions += 1
        
        # Track equity (shares + cash pool)
        total_equity = shares * price_cad + cash_pool
        equity_records.append({"date": dt, "equity": total_equity})
    ...
    years = (prices.index[-1] - prices.index[0]).days / 365.25
    cagr = (end_val / start_val) ** (1 / years) - 1  # Standard CAGR formula
    ...
    return {
        "strategy": f"Market Timing v2 (BUY ONLY - No Selling, Buy RSI<{buy_threshold})",
        ...
        "cagr": cagr,
        "max_drawdown": max_dd,
        "sell_transactions": 0,  # No selling!
        "buy_transactions": rainy_buy_count,
        "total_sold": 0.0,
        "total_bought_back": total_rainy_buys,
        "equity_curve": eq_df,
        "years": years
    }
...
# Simulate market timing strategies (v1 and v2)
print("\n--- Market Timing v1 (Sell on RSI < 45 - WRONG!) ---")
market_timing_v1 = simulate_market_timing_v1(rsi_threshold=45)
print(f"Market Timing v1: CAGR={market_timing_v1['cagr']*100:.2f}%, End=${market_timing_v1['end_equity']:,.0f}, Invested=${market_timing_v1['contributions']:,.0f}, Sells={market_timing_v1['sell_transactions']}")

print("\n--- Market Timing v2 (Sell RSI>70, Buy RSI<45 - CORRECT!) ---")
market_timing_v2 = simulate_market_timing_v2(sell_threshold=70, buy_threshold=45)
print(f"Market Timing v2 (BUY ONLY): CAGR={market_timing_v2['cagr']*100:.2f}%, End=${market_timing_v2['end_equity']:,.0f}, Invested=${market_timing_v2['contributions']:,.0f}, Rainy Buys={market_timing_v2['buy_transactions']}")
...
# Calculate buy-and-hold market return for visualizations
first_price = prices["SPY_CAD"].iloc[0]
...
# =============================================================================
# VISUALIZATION 3 - All 5 Strategies Comparison (v1 vs v2)
# =============================================================================
print("\nGenerating 5-strategy comparison chart...")
fig, ax = plt.subplots(figsize=(16, 8))

# Plot all 5 strategies
# 1. Buy & Hold SPY
buy_hold_equity = prices["SPY_CAD"] * (INITIAL_LUMP_SUM / prices["SPY_CAD"].iloc[0])
ax.plot(prices.index, buy_hold_equity, 
        label=f"Buy & Hold SPY: {buy_hold_cagr*100:.2f}% CAGR", 
...
ax.plot(baseline["equity_curve"].index, baseline["equity_curve"]["equity"], 
        ...
        linewidth=2.5, color='gray', linestyle='--', alpha=0.8)

# 3. Your Variant #2
your_variant = filtered_results[1]  # Rank #2
eq_df = your_variant["equity_curve"]
ax.plot(eq_df.index, eq_df["equity"], 
        label=f"Your Strategy (Bi-weekly $150 RSI<45): {your_variant['cagr']*100:.2f}% CAGR", 
        linewidth=3, color='#1f77b4')

# 4. Market Timing v1 (FAILED - sells on dips)
ax.plot(market_timing_v1["equity_curve"].index, market_timing_v1["equity_curve"]["equity"], 
        label=f"Market Timing v1 (Sell RSI<45 - WRONG!): {market_timing_v1['cagr']*100:.2f}% CAGR", 
        linewidth=2, color='orange', linestyle='-', alpha=0.6)

# 5. Market Timing v2 (BUY ONLY - should match your strategy!)
ax.plot(market_timing_v2["equity_curve"].index, market_timing_v2["equity_curve"]["equity"], 
        label=f"Market Timing v2 (BUY ONLY, RSI<45): {market_timing_v2['cagr']*100:.2f}% CAGR", 
        linewidth=2.5, color='green', linestyle='-.', alpha=0.9)

ax.set_title("Strategy Comparison: Market Timing v1 vs v2 vs DCA vs Rainy Day vs Buy & Hold", 
             fontsize=14, fontweight='bold', pad=15)
...
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.tight_layout()
plt.savefig(out_dir / "all_strategies_comparison.png", dpi=150)
plt.close()
print("Exported: all_strategies_comparison.png")


# =============================================================================
# VISUALIZATION 3B - Your Strategy vs Baseline (Original)
# =============================================================================
...
print(f"    CAGR: {market_timing_v1['cagr']*100:.2f}%")
print(f"    Final equity: ${market_timing_v1['end_equity']:,.0f}")
print(f"    Total invested: ${market_timing_v1['contributions']:,.0f}")
print(f"    Sell transactions: {market_timing_v1['sell_transactions']}")
print(f"    vs Your Strategy: {(market_timing_v1['cagr'] - best['cagr'])*100:+.2f} percentage points")
print(f"    vs Buy & Hold: {(market_timing_v1['cagr'] - buy_hold_cagr)*100:+.2f} percentage points")

print(f"\n  Market Timing v2 (BUY ONLY - No selling!):")
print(f"    CAGR: {market_timing_v2['cagr']*100:.2f}%")
print(f"    Final equity: ${market_timing_v2['end_equity']:,.0f}")
print(f"    Total invested: ${market_timing_v2['contributions']:,.0f}")
print(f"    Sell transactions: {market_timing_v2['sell_transactions']}")
print(f"    Rainy buy transactions: {market_timing_v2['buy_transactions']}")
print(f"    Total rainy buys: ${market_timing_v2['total_bought_back']:,.0f}")
print(f"    vs Your Strategy: {(market_timing_v2['cagr'] - best['cagr'])*100:+.2f} percentage points")
print(f"    vs Market Timing v1: {(market_timing_v2['cagr'] - market_timing_v1['cagr'])*100:+.2f} percentage points")
print(f"    vs Buy & Hold: {(market_timing_v2['cagr'] - buy_hold_cagr)*100:+.2f} percentage points")
print(f"\n  ** Should match Your Strategy since both use same RSI<45 buy logic! **")
