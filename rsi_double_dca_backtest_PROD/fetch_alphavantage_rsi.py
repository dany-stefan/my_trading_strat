...
def fetch_alphavantage_rsi(symbol='SPY', api_key=None):
    ...
    try:
        ...
        # Convert to {date: rsi_value} format, but only keep the most recent 12 days
        result = {}
        # Sort dates descending (most recent first)
        sorted_dates = sorted(rsi_data.keys(), reverse=True)
        for date_str in sorted_dates[:12]:
            result[date_str] = float(rsi_data[date_str]['RSI'])

        print(f"✅ Fetched {len(result)} days of RSI data (most recent 12 days only)")
        return result
