...
│   ├── test_multiple_dates.py            # Test RSI values for specific dates
│   └── test_rsi_ema_nov15.py             # Historical RSI test script
│
├── Shell Scripts
│   └── local_email_send_test.sh          # Send test email locally
│
├── Data Files
│   ├── strategy_tracking.json            # Live tracking (cash pool, rainy buys)
│   └── rsi_vs_sma_payday_analysis.csv    # RSI vs SMA comparison data
│
├── Generated Reports
│   ├── Strategy_Comparisons_with_Variants_Analysis.md  # Comprehensive analysis
│   ├── Summary_Analysis.md                             # Executive summary
│   ├── IMPLEMENTATION_GUIDE.md                         # Setup instructions
│   └── simulated_email_preview.html                    # Local preview
│
└── Generated Charts (PNG)
    └── (Charts attached to emails - generated by separate backtest script)
```
...
| Compare RSI vs SMA | `python compare_rsi_sma_paydays.py` | ❌ No | ❌ No |
| Test specific dates | `python test_multiple_dates.py` | ❌ No | ❌ No |
